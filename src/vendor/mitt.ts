  ...
  return {
    ...
    on(type: keyof Events | '*', handler: GenericEventHandler): void {
      if (type === '*') {
        ...
        const handlers = all.get(type) as
          | EventHandlerList<Events[typeof type]>
          | undefined;
        if (handlers) {
          handlers.push(specificHandler);
        } else {
          all.set(type, [specificHandler] as EventHandlerList<
            Events[typeof type]
          >);
        }
      }
    ...
    emit<Key extends keyof Events>(type: Key, evt?: Events[Key]) {
      ...
      if (!cancelled) {
        ...
        if (lastHandlers) {
          for (const h of (
            lastHandlers as EventHandlerList<Events[keyof Events]>
          ).slice()) {
            h(evt!);
          }
        }
        ...
        if (lastHandlers) {
          for (const h of (
            lastHandlers as WildCardEventHandlerList<Events>
          ).slice()) {
            h(type, evt!);
          }
        }
